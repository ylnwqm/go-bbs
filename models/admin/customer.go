...
type Customer struct {
	...
	Nickname  string    `orm:"column(nickname);size(255);null" description:"真实姓名"`
...
func CustomerLogin(username, password string) (*Customer, bool) {
	...
	cond = cond.AndCond(cond.And("status", 1)).AndCond(cond.And("Username", username).Or("Email", username).Or("Uid", username).Or("Phone", username))
