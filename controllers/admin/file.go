...
// @Param	body		body 	models.File	true		"body for Menu content"
...
func (c *FileController) GetOne() {
	...
	v, _ := admin.GetFileById(id)
	c.Data["Data"] = v

	o := orm.NewOrm()
	var files []*admin.File
	qs := o.QueryTable(new(admin.File))
	qs.All(&files)
	c.Data["File"] = files

	c.TplName = "admin/file-edit.html"
}
...
func (c *FileController) GetAll() {
	...
	l, _ := admin.GetAllFile(query, fields, sortby, order, offset, limit)

	for k, v := range l {
		vv := v.(admin.File)
		vv.DecryptUrl = "/api/v1/download/" + utils.AesEncrypt(vv.Url, utils.Key)
		l[k] = vv
	}
	c.Data["Data"] = l
	c.TplName = "admin/file.html"
	/*if err != nil {
...
func (c *FileController) Delete() {
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.Atoi(idStr)
	if err := admin.DeleteFile(id); err == nil {
		c.Data["json"] = "OK"
	} else {
		c.Data["json"] = err.Error()
	}
