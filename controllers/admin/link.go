...
// LinkController operations for Menu
type LinkController struct {
	BaseController
}

// URLMapping ...
func (c *LinkController) URLMapping() {
...
func (c *LinkController) Post() {
	...
	if _, err := admin.AddLink(&admin.Link{
...
// @router /link/:id [get]
func (c *LinkController) GetOne() {
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.Atoi(idStr)
	v, _ := admin.GetLinkById(id)
	c.Data["Data"] = v

	o := orm.NewOrm()
	var links []*admin.Link
	qs := o.QueryTable(new(admin.Link))
	qs.All(&links)
	c.Data["Link"] = links
...
func (c *LinkController) GetAll() {
	...
	l, _ := admin.GetAllLink(query, fields, sortby, order, offset, limit)

	c.Data["Data"] = l
	c.TplName = "admin/link.html"
	/*if err != nil {
...
func (c *LinkController) Put() {
	...
	if err := admin.UpdateLinkById(&v); err == nil {
		response["msg"] = "修改成功！"
		response["code"] = 200
		c.Data["json"] = response
	} else {
...
func (c *LinkController) Delete() {
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.Atoi(idStr)
	if err := admin.DeleteLink(id); err == nil {
		c.Data["json"] = "OK"
	} else {
		c.Data["json"] = err.Error()
	}
