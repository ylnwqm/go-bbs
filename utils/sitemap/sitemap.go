...
func Sitemap(dir, baseUrl string) {
	...
	c.OnHTML("a", func(e *colly.HTMLElement) {
		href := e.Attr("href")
		if (url[href] == false && strings.Contains(href, baseUrl)) || (strings.HasPrefix(href, "/")  && !strings.Contains(href, ".com") && !strings.Contains(href, ".cn")) {

			if strings.HasPrefix(href, "/") {
				href = baseUrl + href
			}
			url[href] = true
			if !strings.Contains(href, "/api/v1/to") {
