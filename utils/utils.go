...
func Subbbs(source string, l int,url string) string {
	var r = []rune(source)
	length := len(r)
	if l >= length {
		return source
	...
	return string(r[0:(l-4)]) + `...<a href="`+url+`">(展开)</a>`
}

func BbsSubbbs(source string, l int) string {
	var r = []rune(source)
	length := len(r)
	if l >= length {
		return source
...
func GetTopicContent(s string) string {
	r := []rune(s)
	if string(r[0]) != "#" {
		s = "#" + s
	}
...
func DownImage(imgUrl string) (filename string, err error) {
	...
	imgPath := filepath.Join("/static/uploads/images", time.Now().Format("20060102"))
