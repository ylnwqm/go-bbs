...
func FindImg(htmlstr string) []string {
...
func Find9Img(htmlstr string) ([]string,string) {
	var img = make([]string,0)
	doc, _ := goquery.NewDocumentFromReader(strings.NewReader(htmlstr))
	...
	html, _ := doc.Html()
	return img,Html2md(html)
}


// 找出html中的远程img转为本地的img
func HtmlRemoteImg2LocalImg(htmlstr string) string {
	doc, _ := goquery.NewDocumentFromReader(strings.NewReader(htmlstr))
	...
	doc.Find("img").Each(func(i int, selection *goquery.Selection) {
		if src, ok := selection.Attr("src"); ok {
			...
			// 远程照片下载到本地
			localSrc,err := DownImage(src)
			if err == nil{
				src = localSrc
			}else{
				fmt.Printf("远程下载图片到本地错误：%s\n",err.Error())
			}
...
func OlympicsHtml2UrlAndSrc(doc *goquery.Selection) string {
	//doc, _ := goquery.NewDocumentFromReader(strings.NewReader(htmlstr))
	...
	doc.Find("img").Each(func(i int, selection *goquery.Selection) {
		if src, ok := selection.Attr("src"); ok {
			// 远程照片下载到本地
			l := len("../../../")
			src = "https://olympics.com/tokyo-2020/olympic-games/" +  string([]byte(src)[l:])
			fileBaseName := path.Base(src)
			selection.SetAttr("src","https://nihongdengxia.com/static/uploads/images/202107/"+fileBaseName)
			selection.SetAttr("width","26px")
		}
	})
	...
	doc.Find("a").Each(func(i int, selection *goquery.Selection) {
		if href, ok := selection.Attr("href"); ok {
			// 远程照片下载到本地
			l := len("../../../")
			href = "https://olympics.com/tokyo-2020/olympic-games/" +  string([]byte(href)[l:])
